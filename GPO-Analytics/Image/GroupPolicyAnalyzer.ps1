...
If($AllPolicies){
    ...
    ForEach($policy in $AllPolicies){
        ...
        ForEach($gpoPolicySetting in $gpoPolicySettings){
            ...
            $AllSettings += $gpoPolicySetting | Select-Object *,@{n="PolicyName";e={"$($policy.displayName)"}},@{n="PolicyID";e={"$($policy.id)"}},@{n="Compare";e={"$($gpoPolicySetting.settingName)_$($gpoPolicySetting.settingValue)_$($gpoPolicySetting.settingValueType)"}},@{n="DefinitionExist";e={"$($definition)"}}
        }
    ...
    ForEach($all in $AllSettings){
        ...
        if($dupl){
            ...
            $conf = $false
            ForEach($du in $dupl){
                If(("$($du.settingValue)-$($du.settingValueType)") -ne ("$($all.settingValue)-$($all.settingValueType)")){
